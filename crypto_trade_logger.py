...
def ask_position_size_method():
    while True:
        print("\nPosition Size")
        print("1. Manual coin size")
        print("2. Calculate from account risk")
        choice = input("Choose position size method: ").strip()
...
def ask_trade_setup(is_limit_order):
    ...
    r_values = calculate_r_values(direction, entry_price, stop_loss_price)
    account_size = ""
    risk_percent = ""
    risk_dollars = ""

    if ask_position_size_method() == "1":
        position_size = ask_float("Position size in coins: ", minimum=0.00000001)
    else:
        account_size = ask_float("Account size in dollars: ", minimum=0.00000001)
        risk_percent = ask_float("Risk percent (example 1 for 1%): ", minimum=0.00000001)
        risk_dollars = account_size * risk_percent / 100
        max_risk = ask_float(
            "Max dollar risk override (optional, press Enter to skip): ",
            allow_blank=True,
            default="",
            minimum=0.00000001,
        )
        ...
        position_size = risk_dollars / r_values["risk_per_coin"]
...
def show_position_plan(setup, values):
    if setup.get("account_size") == "":
        return

    print("\nPosition Plan")
    print(f"Account size:             ${money(setup['account_size'])}")
    print(f"Risk percent:             {number_text(setup['risk_percent'])}%")
    print(f"Risk dollars:             ${money(setup['risk_dollars'])}")
    print(f"Risk per coin:            {number_text(values['risk_per_coin'])}")
    print(f"Calculated position size: {number_text(setup['position_size'])}")
    print(f"Position value:           ${money(setup['position_value'])}")
    print(f"-1R stop loss:            {number_text(setup['stop_loss_price'])}")
    print(f"1R target:                {number_text(values['take_profit_1r'])}")
    print(f"2R target:                {number_text(values['take_profit_2r'])}")
    print(f"3R target:                {number_text(values['take_profit_3r'])}")
...
def create_trade_record(trade_id, setup, order_type, status):
    ...
    record = {
        ...
        "account_size": setup.get("account_size", ""),
        "risk_percent": setup.get("risk_percent", ""),
        "risk_dollars": setup.get("risk_dollars", ""),
        "position_value": setup.get("position_value", ""),
...
def print_trade_details(trade):
    ...
    if trade.get("account_size", "") != "":
        print(
            f"  Account: ${money(float(trade.get('account_size', 0)))} | "
            f"Risk %: {number_text(trade.get('risk_percent', ''))}% | "
            f"Risk $: ${money(float(trade.get('risk_dollars', 0)))} | "
            f"Value: ${money(float(trade.get('position_value', 0)))}"
        )
...
def new_market_trade(trades):
    ...
    show_position_plan(setup, r_values)
